...
数据分布 $D$ 决定哪些模式频繁进入梯度，优化器长期更新 $\theta$。因此语言比例、事实频率、文体、安全样本和损失权重都能形成参数先验。
...
这使模型成为一个有路径依赖的动力系统。两个首 token 即使只差一个字符，也可能把后续计算带到不同区域。
...
如果第一步强制输出 `ANSWER`，这个 token 又会强化“继续回答”的上下文；若强制输出 `REFUSE`，则后续更容易延续拒绝句式。模型行为的巨大差异并不要求参数变化。
...
- C03 中固定矩阵是否对不同输入产生不同特征和 top token；
