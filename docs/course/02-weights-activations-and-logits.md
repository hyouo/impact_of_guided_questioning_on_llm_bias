...
这一章建立了精确的局部数学关系，但不能单独回答“真实模型为什么形成某个语义行为”。具体来说：

- $c_{ij}=w_{ij}x_j$ 是直接局部贡献，不包含门控、归一化、并行路径和下游抵消；
- softmax 赔率恒等式描述给定 logits 后的关系，不解释 logits 是由哪条内部回路产生的；
- 两个 token 的赔率变化不保证其中一个成为全词表 top-1，也不保证最终被采样；
- C01 的 effectiveness 是教学代理，不能据此估计前沿模型中无效或有害权重的比例；
- steering 成功只说明可操纵性，不能代替自然运行中的必要性和机制忠实性证据。

因此，这一章允许你否定“最大权重必然最重要”等一般推理，却不允许从一个数值例子直接给真实模型中的参数赋予完整功能标签。
