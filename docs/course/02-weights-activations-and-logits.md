...
- 推导两个 token 的相对赔率；
- 解释为什么小的隐藏状态变化可能翻转首 token，却不必夸大为“一个神经元控制了模型”。
...
低温放大相同 logit 差，高温压缩它。但赔率变化不代表 A 一定被采样，因为词表里还有其他 token，解码还可能使用 top-p、top-k 或 greedy。
...
“隐藏状态变化很小”不是完整判断；关键是变化是否对准了竞争 token 的差分方向。
...
C01 比较“大而罕见”的连接与“小而常见”的连接；C02 直接验证赔率恒等式和 $e$ 倍关系。
