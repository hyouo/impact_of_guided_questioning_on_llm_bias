...
它同时混合了 tokenizer、位置、语义、Attention、策略和行为，也预设了原因。
...
- H1：换行改变 token 序列；
- H2：token 差异导致某层状态显著改变；
- H3：该状态对目标 logit 具有因果作用；
- H4：效应跨模板和模型稳定；
- H5：安全行为变化由同一中介解释。

H1 是观测，H3 需要 patching，H4 需要复现，H5 需要中介和替代解释比较。不要一次实验就跳到 H5。
...
不要在看完结果后选择最漂亮的图。提前写下目标 token 或 logit difference、层/位置/头、距离或恢复指标、样本数和 seed、聚合方式和置信区间、多重比较修正，以及成功、失败和不确定阈值。
